 ...
 *   connectionFromModel:                       //default is false, gets the connectionUri from the model
...
async function mountDocToSave ({ options, params, doc }, method) {
  ...
  // To handle dot notation we need to parse the properties before it goes to diffing
  // else once it gets to the diff it won't show the previous value
  let old = _toJSON(get(result, 'old', {}))
  old = expandRootDotProperties(old)

  let current = _toJSON(get(result, 'current', {}))
  current = expandRootDotProperties(current)

  set(result, 'old', old)
  set(result, 'current', current)
...
async function _helperPreUpdate (result) {
  ...
  let old = undefined
  if(hasMongoDuplicateProtection(this)) {
    old = _toJSON(await self.findOne(query).clone())
  } else {
    old = _toJSON(await self.findOne(query))
  }
...
async function _processCreateHistory (params, options) {
  ...
  setConnectionStringFromModel(options, this)

  await _saveHistoryLog(docMapped, options)
}

function setConnectionStringFromModel(options, self) {
  const connectionFromModel = get(options, 'connectionFromModel')

  if(connectionFromModel) {
    options.connectionUri = self.model.db._connectionString
  }
}
...
async function _processRemoveMany (options) {
  ...
  let olds = undefined
  if(hasMongoDuplicateProtection(this)) {
    olds = _toJSON(await this.find(query).clone())
  } else {
    olds = _toJSON(await this.find(query))
  }
...
async function _processPreRemove (options) {
  ...
  let old = undefined
  if(hasMongoDuplicateProtection(this)) {
    old = _toJSON(await this.findOne(query).clone())
  } else {
    old = _toJSON(await this.findOne(query))
  }
...
function hasMongoDuplicateProtection(self) {
  const mongoVersion = self.model.db._connectionOptions.driverInfo.version
  const majorVersion = mongoVersion.split(".")[0]
...
function expandRootDotProperties(obj){
  const convertDotNotationToObjects = function (target, dotNotation, value) {
    const [k, ...rest] = dotNotation.split('.')
    if (rest.length == 0) {
      target[k] = value
      return target
    }
    if (!target[k]) target[k] = {}
    target[k] = convertDotNotationToObjects(target[k], rest.join('.'), value)
    return target
  }
  for (const [k, v] of Object.entries(obj)) {
    delete obj[k]
    obj = convertDotNotationToObjects(obj, k, v)
  }
  return obj
}
