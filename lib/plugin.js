...
async function mountDocToSave ({ options, params, doc }, method) {
  ...
  old = parseDotProperties(old)

  let current = _toJSON(get(result, 'current', {}))
  current = parseDotProperties(current)
...
function getObject(target, dotNotation, value) {
  ...
  target[k] = getObject(target[k], rest.join('.'), value)
  return target
}

function parseDotProperties(obj) {
  for (const [k, v] of Object.entries(obj)) {
    delete obj[k]
    obj = getObject(obj, k, v)
  }
  return obj
}
